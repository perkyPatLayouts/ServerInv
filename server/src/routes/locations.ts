...
router.post("/", requireAdmin, validate(locationSchema), async (req: Request, res: Response) => {
  const [row] = await db.insert(locations).values(req.body).returning();
  res.status(201).json(row);
});

router.put("/:id", requireAdmin, validate(locationSchema), async (req: Request, res: Response) => {
