...
import { billingPeriods } from "../db/schema/index.js";
...
router.post("/", requireEditorOrAdmin, validate(billingPeriodSchema), async (req: Request, res: Response) => {
  const [row] = await db.insert(billingPeriods).values(req.body).returning();
  res.status(201).json(row);
});

/** PUT /api/billing-periods/:id */
router.put("/:id", requireEditorOrAdmin, validate(billingPeriodSchema), async (req: Request, res: Response) => {
