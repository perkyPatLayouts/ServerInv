...
import { serverTypes } from "../db/schema/index.js";
...
router.post("/", requireEditorOrAdmin, validate(serverTypeSchema), async (req: Request, res: Response) => {
  const [row] = await db.insert(serverTypes).values(req.body).returning();
  res.status(201).json(row);
});

router.put("/:id", requireEditorOrAdmin, validate(serverTypeSchema), async (req: Request, res: Response) => {
