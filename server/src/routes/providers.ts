...
router.post("/", requireEditorOrAdmin, validate(providerSchema), async (req: Request, res: Response) => {
  const [row] = await db.insert(providers).values(req.body).returning();
  res.status(201).json(row);
});

router.put("/:id", requireEditorOrAdmin, validate(providerSchema), async (req: Request, res: Response) => {
